...
<p align="justify">As disaster response action plans are typically organized within specific jurisdictions, it is reasonable to assume that the relief system aligns with the geographic boundaries of the jurisdictional area. With limited access to data regarding relief centers and street information, we utilized OpenStreetMap, a publicly available API, to create the necessary path network. OpenStreetMap allows users to view, edit, and use geographic data freely. The map data is contributed by individuals, organizations, and government agencies, making it a suitable mapping resource. In addition to the simple road network, it provides attributes of the roads needed for the project, including, but not limited to, the number of lanes and the maximum allowed speed.</p>

<p align="justify">For our optimization problem, which requires finding k paths, we determined that limiting the number of connections between specific nodes (a, b) to 3 provides the best trade-off between calculation time and complexity. Choosing k = 10 would have included paths that take large detours, which seem unlikely even in an emergency. Furthermore, we selected five Relief Centers (RC) and one Distribution Center (DC) for the same reason. The locations of these centers were set to be equally distributed on the map to cover most of the households affected. If the algorithm were implemented by authorities, the real coordinates of the centers would have to be used in the program.</p>

<p align="justify">To find the three shortest paths, we employed Dijkstra's Algorithm, a graph traversal and pathfinding algorithm that efficiently determines the shortest path between two nodes in a weighted graph. The algorithm maintains a set of explored nodes and continually selects the node with the smallest tentative distance from the source node, updating the distances to its neighbors. The algorithm guarantees optimality by greedily choosing the shortest paths first. The use of a priority queue ensures that the algorithm efficiently explores nodes with the shortest tentative distances first. This algorithm proved particularly useful in our networking and transportation application, where finding the three shortest routes between two points was our primary goal.</p>


### Analysis
<p align="justify">We apply the Sample Average Approximation method to analyze this problem. The SAA method for solving stochastic optimization problems involves generating a set of samples from the probability distribution representing uncertainties, such as the impact of earthquakes on road travel times. In our analysis, we generate 30 samples from a road damage probability density function proposed in the reference paper. The road damage probability density function is as follows:  
